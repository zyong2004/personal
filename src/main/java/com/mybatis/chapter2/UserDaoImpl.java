...
public class UserDaoImpl implements IUserDao {
	...
	public User findbyid(String id) {
		SqlSession session = MyBatisUtils.getSqlSession(Constant.CONFIG_PATH);
		User user = session.selectOne("com.mybatis.chapter1.IUserDao.selectUSER",Integer.valueOf(id));
		MyBatisUtils.closeSession(session);
		return user;
	}
	...
	public List<User> findByName(String name) {
		SqlSession session = MyBatisUtils.getSqlSession(Constant.CONFIG_PATH);
		List<User> users = session.selectList("com.mybatis.chapter2.IUserDao.selectByName",name);
		MyBatisUtils.closeSession(session);
		return users;
	}
	...
	public List<User> findLike(User user) {
		SqlSession session = MyBatisUtils.getSqlSession(Constant.CONFIG_PATH);
		List<User> users = session.selectList("com.mybatis.chapter2.IUserDao.selectLike",user);
		MyBatisUtils.closeSession(session);
		return users;
	}
